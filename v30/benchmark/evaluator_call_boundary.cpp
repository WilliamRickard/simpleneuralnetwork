...
#define V30B_ACCUM(K,G0,G1) do { \
    const __m512d xv=_mm512_set1_pd(xq[(K)]); \
    (G0)=_mm512_fmadd_pd(xv,hiddenDeltaLow,(G0)); \
    (G1)=_mm512_fmadd_pd(xv,hiddenDeltaHigh,(G1)); \
} while(false)

#define V30B_ACCUM_ALL() do { \
    V30B_ACCUM(0,g00,g01); V30B_ACCUM(1,g10,g11); V30B_ACCUM(2,g20,g21); \
    V30B_ACCUM(3,g30,g31); V30B_ACCUM(4,g40,g41); V30B_ACCUM(5,g50,g51); \
    V30B_ACCUM(6,g60,g61); V30B_ACCUM(7,g70,g71); V30B_ACCUM(8,g80,g81); \
    V30B_ACCUM(9,g90,g91); V30B_ACCUM(10,g100,g101); \
} while(false)

#define V30B_STORE(G) do { \
...
/** V29 grouped scheduling plus V28's explicit preactivation call boundary. */
__attribute__((target("avx512f,avx512dq,fma")))
static void evaluateSliceCallBoundaryV30(const Dataset&data,size_t firstRow,size_t lastRow,
    ...
    while(row+V17_FORWARD_ROWS<=lastRow){
        ...
        for(size_t rr=0;rr<groupRows;rr++){
            ...
            const __m512d hiddenDeltaLow=_mm512_mul_pd(
                _mm512_mul_pd(d,wTwoLow),_mm512_mul_pd(hiddenLow,_mm512_sub_pd(one,hiddenLow)));
            const __m512d hiddenDeltaHigh=_mm512_mul_pd(
                _mm512_mul_pd(d,wTwoHigh),_mm512_mul_pd(hiddenHigh,_mm512_sub_pd(one,hiddenHigh)));
...
static V14Evaluation evaluateCallBoundaryV30(const Dataset&data,size_t startRow,size_t batchSize,
...
#pragma omp parallel num_threads(static_cast<int>(threads))
    {
        ...
        evaluateSliceCallBoundaryV30(data,startRow+offset,startRow+offset+count,network,partials[tid]);
    }
#else
    evaluateSliceCallBoundaryV30(data,startRow,startRow+batchSize,network,partials[0]);
#endif
...
static Dataset makeBoundaryDataV30(size_t rows,unsigned seed){
    Dataset data;
...
static Network makeBoundaryNetworkV30(unsigned seed){
    Network network;
    const double s=static_cast<double>(seed+1);
    for(size_t k=0;k<NUMBER_OF_VARIABLES;k++)
...
static bool sameEvaluationBoundaryV30(const V14Evaluation&a,const V14Evaluation&b){
...
template<class F>
static double timeBoundaryV30(F&&function){
...
static double medianBoundaryV30(vector<double> values){
...
int main(){
    ...
    for(unsigned seed=0;seed<5;seed++){
        Dataset data=makeBoundaryDataV30(100000,seed);
        Network network=makeBoundaryNetworkV30(seed);
        ...
        for(size_t rows:exactRows)for(size_t workers=1;workers<=5;workers++){
            ...
            const V14Evaluation b=evaluateCallBoundaryV30(data,0,rows,n30,parameters,workers);
            if(!sameEvaluationBoundaryV30(a,b)){
    ...
    Dataset data=makeBoundaryDataV30(100000,2);
    Network base=makeBoundaryNetworkV30(2),candidate=base;
    ...
    for(size_t workers:vector<size_t>{1,2})for(size_t rows:vector<size_t>{5000,10000,20000,50000,100000}){
        ...
        for(int pair=0;pair<15;pair++){
            ...
            double a=0.0,b=0.0;
            if((pair&1)==0){
                a=timeBoundaryV30([&]{return evaluateV29(data,0,rows,base,parameters,workers);});
                b=timeBoundaryV30([&]{return evaluateCallBoundaryV30(data,0,rows,candidate,parameters,workers);});
            }else{
                b=timeBoundaryV30([&]{return evaluateCallBoundaryV30(data,0,rows,candidate,parameters,workers);});
                a=timeBoundaryV30([&]{return evaluateV29(data,0,rows,base,parameters,workers);});
            }
        ...
        cout<<"BOUNDARY rows="<<rows<<" workers="<<workers
            <<" median_speedup="<<medianBoundaryV30(speedups)
            <<" wins="<<count_if(speedups.begin(),speedups.end(),[](double x){return x>1.0;})<<"/15\n";
    }
    return 0;
}

#undef V30B_DECLARE_W1
#undef V30B_ACCUM
#undef V30B_ACCUM_ALL
#undef V30B_STORE
