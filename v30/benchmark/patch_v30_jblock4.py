...
direct_output = reduced_output.replace("total.values", "out.values")
direct_output4 = reduced_output4.replace("total.values", "out.values")
direct_w1 = reduced_w1.replace("total.values", "out.values").replace("startRow+offset", "blockStart")
direct_w14 = reduced_w14.replace("total.values", "out.values").replace("startRow+offset", "blockStart")
...
for old,new,label in (
    ...
    if count != 1:
        raise SystemExit(f"{label} loop not found after parameter-major patch: {count}")
