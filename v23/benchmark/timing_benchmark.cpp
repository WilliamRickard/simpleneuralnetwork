...
static double gammaScaled(const std::deque<Pair>&h,const std::array<double,PARAMS>&scale,int mode){if(h.empty())return 1.0;const auto&s=h.back().s;const auto&y=h.back().y;if(mode==0)return gammaFromHist(h);double sy=dot(s,y),yDy=0,qyS=0,qy2=0,sDinvS=0;for(size_t i=0;i<PARAMS;i++){double q=scale[i],qy=q*y[i];yDy+=y[i]*qy;qyS+=qy*s[i];qy2+=qy*qy;if(q>0)sDinvS+=s[i]*s[i]/q;}double g=1.0;if(mode==1&&yDy>0)g=sy/yDy;else if(mode==2&&qy2>0)g=qyS/qy2;else if(mode==3&&yDy>0&&sDinvS>0)g=std::sqrt(sDinvS/yDy);if(!(g>0)||!std::isfinite(g))g=gammaFromHist(h);return g;}
...
int main(int argc,char**argv){
    ...
    const auto dataStart=totalStart;
    Data data=makeData(rows);
    const auto dataEnd=std::chrono::steady_clock::now();
    auto p=initP();
    const auto optStart=std::chrono::steady_clock::now();
    Eval cur=evaluate(data,p,threads);size_t evals=1,gnp=0,u=0;std::deque<Pair>hist;bool deep=false,have=false,dense=false;std::array<double,PARAMS>scale{};std::vector<double>H;bool r0225=false,r01=false,r002=false;
    while(cur.pe>target&&u<maxU){if(cur.pe<=.05)deep=true;if(deep&&!have){scale=gnScale(data,p,threads);have=true;gnp++;}if(have&&!dense){if(!r0225&&cur.pe<=.0225){scale=gnScale(data,p,threads);gnp++;r0225=true;}if(!r01&&target<=.001&&cur.pe<=.01){scale=gnScale(data,p,threads);gnp++;r01=true;}if(!r002&&target<=.0005&&cur.pe<=.002){scale=gnScale(data,p,threads);gnp++;r002=true;}if(cur.pe<=sw){initDense(hist,scale,H);dense=true;}}size_t lim=deep?HIST_DEEP:HIST_SHALLOW;std::array<double,PARAMS>d=dense?denseDir(cur.g,H):direction(cur.g,hist,have?&scale:nullptr);double dg=dot(cur.g,d);if(!(dg<0)&&!dense){for(size_t i=0;i<PARAMS;i++)d[i]=-cur.g[i];dg=-dot(cur.g,cur.g);hist.clear();}if(!(dg<0)&&dense){initDense(hist,scale,H);d=denseDir(cur.g,H);dg=dot(cur.g,d);if(!(dg<0)){for(size_t i=0;i<PARAMS;i++)d[i]=-cur.g[i];dg=-dot(cur.g,cur.g);}}bool ok=false;double step=1;std::array<double,PARAMS>cand{};Eval nxt;for(size_t ls=0;ls<MAX_LS;ls++){for(size_t i=0;i<PARAMS;i++)cand[i]=p[i]+step*d[i];nxt=evaluate(data,cand,threads);evals++;if(nxt.obj<=cur.obj+ARMIJO*step*dg){ok=true;break;}step*=.5;}if(!ok)break;Pair pair;for(size_t i=0;i<PARAMS;i++){pair.s[i]=cand[i]-p[i];pair.y[i]=nxt.g[i]-cur.g[i];}double sy=dot(pair.s,pair.y);if(acceptCurv(pair.s,pair.y,sy)){if(dense)updateDense(pair,H);else{if(hist.size()>=lim)hist.pop_front();pair.rho=1/sy;hist.push_back(pair);}}p=cand;cur=nxt;u++;}
    const auto optEnd=std::chrono::steady_clock::now();
    const auto totalEnd=optEnd;
    const double dataSec=std::chrono::duration<double>(dataEnd-dataStart).count();
    const double optSec=std::chrono::duration<double>(optEnd-optStart).count();
    const double totalSec=std::chrono::duration<double>(totalEnd-totalStart).count();
    std::cout<<std::setprecision(15)<<"rows="<<rows<<" target="<<target<<" switch="<<sw<<" gamma="<<GAMMA_MODE<<" updates="<<u<<" evals="<<evals<<" gn="<<gnp<<" total="<<(evals+gnp)<<" pe="<<cur.pe<<" obj="<<cur.obj<<" data_s="<<dataSec<<" opt_s="<<optSec<<" total_s="<<totalSec<<"\n";
}
