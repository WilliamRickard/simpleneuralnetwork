 ...
 * ==========================
 *
 * V4 keeps the same 11 -> 16 -> 1 sigmoid network and training equations as
 * v3. It processes observations in small blocks so the expensive sigmoid calls
 * can use a vector-math backend when one is available. Parallel training also
 * keeps one OpenMP team alive across gradient descents instead of creating a new
 * team for every update.
 *
 * The portable build falls back to the scalar sigmoid. On glibc/x86 systems,
 * defining SIMPLE_NN_USE_LIBMVEC enables runtime-dispatched AVX2/AVX-512 vector
 * exp() calls through glibc libmvec while retaining scalar handling for extreme
 * values and unsupported CPUs.
 */
...
/*
 * Process one block of observations. Hidden pre-activations for the block are
 * contiguous, allowing the sigmoid stage to use vector exp() when available.
 * After W2 has used an activation, the same storage is reused for deltaTwo.
 */
static void processTrainingBlock(const Dataset &data, size_t startRow, size_t rowCount, const Network &network, ThreadAccumulator &accumulator) {
...
static Metrics calculateGradientsBlocked(const Dataset &data, size_t startRow, size_t rowCount, const Network &network, Gradients &gradients) {
    ...
    for(size_t offset=0;offset<rowCount;offset+=BLOCK_SIZE) {
        ...
        processTrainingBlock(data,startRow + offset,rowsInBlock,network,accumulator);
    }
...
#pragma omp for schedule(static)
                for(long long block=0;block<static_cast<long long>(blockCount);block++) {
                    ...
                    processTrainingBlock(data,startRow + offset,rowsInBlock,network,accumulators[thread]);
                }
