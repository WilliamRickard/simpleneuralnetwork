 ...
 * ==========================
 *
 * V8 keeps the same 11 -> 16 -> 1 sigmoid network and training equations as
 * v7. Single-thread training removes redundant hidden-scratch initialisation,
 * resolves the vector sigmoid backend once, and processes the whole row range
 * through one AVX-512 driver. Once a lightweight pass has accumulated enough
 * percentage error that the target cannot be met, later rows skip percentage
 * divisions while continuing to calculate the exact gradients.
 *
 * Parallel training keeps v7's scheduling and arithmetic for small and medium
 * batches. On large batches it keeps the same arithmetic but caches the vector
 * sigmoid dispatch, which benchmarked better than changing the parallel dataflow.
 * AVX2/FMA and portable scalar paths remain available as fallbacks.
 */
...
constexpr size_t V8_SINGLE_THRESHOLD = 50000;
constexpr size_t V7_PARALLEL_THRESHOLD = 50000;
constexpr size_t V8_PARALLEL_THRESHOLD = 1000000;
...
using SigmoidVectorKernel = void (*)(double *, size_t);
static SigmoidVectorKernel selectSigmoidVectorKernel() {
#if defined(SIMPLE_NN_USE_LIBMVEC) && defined(__GLIBC__) && defined(__x86_64__) && defined(__GNUC__)
    if(__builtin_cpu_supports("avx512f")) return sigmoidVectorAVX512;
    if(__builtin_cpu_supports("avx2")) return sigmoidVectorAVX2;
#endif
    return nullptr;
}
static void sigmoidVectorCached(double *values, size_t count) {
    static const SigmoidVectorKernel kernel = selectSigmoidVectorKernel();
    if(kernel != nullptr) { kernel(values,count); return; }
    for(size_t i=0;i<count;i++) values[i] = sigmoid(values[i]);
}
...
__attribute__((target("avx512f,avx512dq,fma"),always_inline)) static inline void processTrainingTileAVX512V8(const Dataset &data, size_t startRow, size_t rowCount, const Network &network, ThreadAccumulator &accumulator, bool detailed) {
    alignas(64) array<double,BLOCK_SIZE*HIDDEN_NODES> hidden;
    alignas(64) array<double,BLOCK_SIZE> output,deltaThree,percentage;
    const __m512d one=_mm512_set1_pd(1.0),hundred=_mm512_set1_pd(100.0),signMask=_mm512_set1_pd(-0.0);
    ...
    sigmoidVectorCached(hidden.data(),rowCount*HIDDEN_NODES);

    const __m512d w20=_mm512_loadu_pd(network.wTwo.data()),w21=_mm512_loadu_pd(network.wTwo.data()+8);
    for(size_t i=0;i<rowCount;i++){const double*h=hidden.data()+i*HIDDEN_NODES;const __m512d sum=_mm512_fmadd_pd(_mm512_loadu_pd(h+8),w21,_mm512_mul_pd(_mm512_loadu_pd(h),w20));output[i]=_mm512_reduce_add_pd(sum);} sigmoidVectorCached(output.data(),rowCount);
    ...
    if(CalculatePercentage) {
        ...
        for(;i<rowCount;i++) {
            const double prediction=output[i],error=prediction-data.y[startRow+i];
            deltaThree[i]=error*prediction*(1.0-prediction);
        }
    }
...
__attribute__((target("avx512f,avx512dq,fma"))) static void processTrainingRangeAVX512V8(const Dataset &data, size_t startRow, size_t rowCount, const Network &network, ThreadAccumulator &accumulator, bool detailed, double percentageLimit) {
    for(size_t offset=0;offset<rowCount;offset+=BLOCK_SIZE) {
        const size_t rowsInBlock=min(BLOCK_SIZE,rowCount-offset);
        if(detailed || accumulator.percentageErrorSum<=percentageLimit) processTrainingTileAVX512V8<true>(data,startRow+offset,rowsInBlock,network,accumulator,detailed);
        else processTrainingTileAVX512V8<false>(data,startRow+offset,rowsInBlock,network,accumulator,false);
    }
}

__attribute__((target("avx512f,avx512dq,fma"),unused)) static void processTrainingBlockAVX512V8Parallel(const Dataset &data, size_t startRow, size_t rowCount, const Network &network, ThreadAccumulator &accumulator, bool detailed) {
    alignas(64) array<double,BLOCK_SIZE*HIDDEN_NODES> hidden{};
    alignas(64) array<double,BLOCK_SIZE> output{},deltaThree{},percentage{};
    const __m512d one=_mm512_set1_pd(1.0),hundred=_mm512_set1_pd(100.0),signMask=_mm512_set1_pd(-0.0);
...
static bool supportsV8Kernel() {
#if defined(__x86_64__) && defined(__GNUC__)
    static const bool supported=__builtin_cpu_supports("avx512f") && __builtin_cpu_supports("avx512dq") && __builtin_cpu_supports("fma");
    return supported;
#else
    return false;
#endif
...
#if defined(__x86_64__) && defined(__GNUC__)
    if(useV7 && supportsV8Kernel()) return processTrainingBlockAVX512V7;
...
TrainResult trainRange(const Dataset &data, size_t startRow, size_t batchSize, size_t batchIndex, Network &network, const OptimiserConfig &config) {
    ...
    if(config.threads > 1) {
        ...
        const bool useV8Parallel = batchSize >= V8_PARALLEL_THRESHOLD && supportsV8Kernel();
        const TrainingBlockKernel kernel=useV8Parallel?processTrainingBlockAVX512V8Parallel:selectTrainingBlockKernel(useV7);
...
#pragma omp single
...
#endif
    {
        ...
        while(true) {
            ...
            if(supportsV8Kernel() && batchSize>=V8_SINGLE_THRESHOLD) {
                ...
                processTrainingRangeAVX512V8(data,startRow,batchSize,network,accumulator,detailed,percentageLimit);
                result.metrics=combineAccumulators(accumulators,gradients,batchSize,detailed);
            }
            else result.metrics=calculateGradientsBlocked(data,startRow,batchSize,network,gradients,detailed);
            const bool targetReached=result.metrics.percentageError<=config.percentageErrorTarget;
            const bool updateLimitReached=result.updates>=config.maxDescents;
            if(targetReached && !detailed) result.metrics=calculateMetrics(data,startRow,batchSize,network);
            if(logNow || targetReached || updateLimitReached) printProgress(batchIndex,result.updates,result.metrics);
